...
const currentTitle = computed(() => {
  if (route.name === 'custom-data-create') return '新增自定义数据'
  if (route.name === 'custom-data-edit') return '编辑自定义数据'
  if (route.name === 'custom-data-help') return '使用说明'
  return '自定义数据管理'
})

const sideItems = [
  { label: '数据列表', path: '/custom-data/list', names: ['custom-data-list'] },
  { label: '新增数据', path: '/custom-data/create', names: ['custom-data-create'] },
  { label: '使用说明', path: '/custom-data/help', names: ['custom-data-help'] },
]
  ...
  <div class="vs-ref-shell">
    <div class="vs-ref-frame auto-frame">
      <aside class="auto-side">
        <div class="auto-side-head">自定义数据</div>
